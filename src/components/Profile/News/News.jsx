...
const PLATFORMS = {
  ...
  youtube: {
    ...
    icon: '▶️',
    color: '#FF0000',
    badgeColor: 'linear-gradient(135deg, #FF0000 0%, #CC0000 100%)',
    settings: {
      maxMediaFiles: 1,
      allowedMediaTypes: ['video/mp4', 'video/quicktime', 'video/x-matroska'],
      maxFileSizeMB: 256,
      supportsStories: false,
      supportsReels: false,
      supportsShorts: true,
  ...
  twitter: {
    ...
    icon: '🐦',
    color: '#000000',
    badgeColor: 'linear-gradient(135deg, #000000 0%, #1DA1F2 100%)',
    settings: {
      maxMediaFiles: 4,
      ...
      maxTextLength: 280,
      recommendedImageResolution: '1600x900',
      recommendedVideoResolution: '1280x720',
    ...
    <div className="media-upload-section">
      ...
      {mediaFiles.length > 0 && (
        <div className="media-preview-grid">
          ...
          {onClearAll && mediaFiles.length > 0 && (
            <button 
              type="button" 
              className="media-clear-all"
...
const EditPostModal = ({ post, platform, onClose, onSave, onDelete }) => {
  ...
  const handleDelete = () => {
    if (window.confirm(`Ви впевнені, що хочете видалити публікацію "${formData.title}"?`)) {
      setIsDeleting(true);
      setTimeout(() => {
        onDelete?.(post.id);
        onClose();
  ...
  const handleCopyLink = async () => {
    const link = `https://app.example.com/draft/${post?.id}`;
    try {
  ...
  const statusOptions = [
    { value: '', label: 'Оберіть статус', disabled: true },
  ...
  const audienceOptions = [
    { value: '', label: 'Оберіть аудиторію', disabled: true },
  ...
  const settingsTabs = [
    ...
    { id: 'schedule', label: 'Публікація' },
    { id: 'advanced', label: 'Додатково' }
  ];
            ...
            <div id="panel-content" role="tabpanel" aria-labelledby="tab-content">
              ...
              <div className="form-group">
                <textarea
                  ...
                  placeholder="Опис змісту публікації *"
                  rows={4}
                  aria-required="true"
                ...
                <ul>
                  {platform?.id === 'instagram' && (
                    <>
                      ...
                      <li>Stories: 9:16, до 15 секунд на слайд</li>
                    </>
                  )}
                  ...
                  {platform?.id === 'tiktok' && (
                    <>
                      ...
                      <li>Оптимальна тривалість: 15-60 секунд</li>
                      <li>Використовуйте трендові звуки для більшого охоплення</li>
                    </>
                  )}
                  {platform?.id === 'youtube' && (
                    <>
                      <li>Рекомендоване співвідношення: 16:9 (1920x1080)</li>
                      <li>Shorts: вертикальне відео 9:16, до 60 секунд</li>
                      <li>Мінімальна тривалість: 12 секунд, максимальна: 12 годин</li>
                      <li>Підтримувані формати: MP4, MOV, MKV</li>
                    </>
                  )}
                  {platform?.id === 'twitter' && (
                    <>
                      <li>Максимальна довжина тексту: 280 символів</li>
                      <li>До 4 медіафайлів в одному твіті (зображення, відео)</li>
                      <li>Рекомендоване співвідношення: 16:9 або 1:1 для зображень</li>
                      <li>Відео: до 2 хвилин 20 секунд для звичайних користувачів</li>
                    </>
                  )}
                </ul>
              </div>
            ...
            <div id="panel-advanced" role="tabpanel" aria-labelledby="tab-advanced">
              ...
              <div className="form-group">
                <div className="input-with-copy">
                  <input
                    ...
                    placeholder="Посилання на чернетку"
                    readOnly
                  />
                  <button 
                    type="button" 
              ...
              <div className="danger-zone">
                <button 
                  ...
                  disabled={isDeleting}
                >
                  {isDeleting ? 'Видалення...' : 'Видалити публікацію'}
                </button>
              </div>
            </div>
          )}
...
const EmptyState = ({ platformName, onCreatePost }) => (
...
const News = () => {
  ...
  const handleSavePost = useCallback((updatedPost) => {
    if (isCreating) {
      ...
      console.log('Новий пост створено:', updatedPost);
    } else {
      setPosts(prevPosts => 
  ...
  const handleDeletePost = useCallback((postId) => {
    if (window.confirm('Ви впевнені, що хочете видалити цю публікацію?')) {
      ...
      console.log('Пост видалено:', postId);
    }
  }, [setPosts]);
      ...
      >
        ...
        {filteredPosts.length === 0 && (
          <EmptyState 
            ...
            onCreatePost={activePlatform !== 'all' ? handleCreatePost : undefined}
          />
        )}
      </section>
